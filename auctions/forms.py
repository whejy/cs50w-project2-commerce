...
class NewListForm(forms.Form):
    ...
    start_bid = forms.DecimalField(
        required=True, decimal_places=2, min_value=0.00, widget=forms.NumberInput(attrs={
        'value': '0.00', 'step': STEP
        }), label='Starting Bid $'
    )
    image = forms.URLField(
        required=False, label='Image URL'
    )
...
class NewBidForm(forms.Form):
    bid = forms.DecimalField(
        required=False, decimal_places=2, widget=forms.NumberInput(attrs={
        'step': STEP
        }), label='$'
    )        
